#!/usr/bin/env python3
"""
H8391001 Daily Order Report Automation
Runs as GitHub Actions cron job at 9am HKT daily.

Steps:
1. Login to MMS (merchant.shoalter.com)
2. Download TODAY's partial report for H8391001
3. Download YESTERDAY's final report for H8391001
...
async def set_store_and_date(page, target_date: datetime):
    """Select store H8391001 and set the report date."""
...
def _norm(s):
    return s.replace("H8391001_S_", "")

def _pfx(bare):
    return bare if bare.startswith("H8391001_S_") else "H8391001_S_" + bare
...
async def main():
    ...
    print("=" * 55)
    print(f"H8391001 Daily Order Report — {now.strftime('%Y-%m-%d %H:%M HKT')}")
